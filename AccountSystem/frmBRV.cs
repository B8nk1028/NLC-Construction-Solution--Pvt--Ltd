...
{
    public partial class frmBRV : Form
    {
        ...
        private static frmBRV defaultInstance;
        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static frmBRV Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new frmBRV();
        ...
        private void fillHeads_R()
        {
            ...
            cboAcc.Text = null; cbRef.Text = null; cbRef2.Text = null;
        }
        ...
        private void frmBVoucher_Load(object sender, EventArgs e)
        {
            ...
            Fn.fillCombo(cboWHTax, "SELECT  EntryAccID, EntryAccName FROM TblAccEntry WHERE (EntryAccName LIKE N'%w%tax%')");
            ...
            Fn.GetNum(txtVoucherNo, "SELECT COUNT(tbl.VoucherID) AS Voch FROM (SELECT VoucherID FROM TblVoucherDTL GROUP BY VoucherID) AS tbl INNER JOIN TblVoucherHDR ON tbl.VoucherID = TblVoucherHDR.VoucherID WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'BRV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");
            txtVoucherNo.Text = "BRV/" + txtVoucherNo.Text;fillHeads_E();
            ...
            else
            {
                if (ValidateControls())
                {
                    ...
                    else
                    {
                        ...
                        items = items + "," + "{\"AccEntryID\":" + JsonConvert.ToString(cmboBank.SelectedValue) + ",\"Description\":" + JsonConvert.ToString((desc.Substring(1))) + ",\"AmountDeb\":" + JsonConvert.ToString(txtAmount.Text) + ",\"AmountCre\":" + JsonConvert.ToString(0) + ",\"Ref\":"+JsonConvert.ToString(cbRef2.SelectedValue) + "}";
                        if (chkWHT.Checked)
                        {
                            items = items + "," + "{\"AccEntryID\":" + cboWHTax.SelectedValue + ",\"Description\":" + JsonConvert.ToString((desc.Substring(1))) + ",\"AmountDeb\":" + JsonConvert.ToString(txtWHT.Text) + ",\"AmountCre\":" + JsonConvert.ToString(0) + ",\"Ref\":" + JsonConvert.ToString(cbRef2.SelectedValue) + "}";
                        }
                        if (chkTAX.Checked)
                        {
                            items = items + "," + "{\"AccEntryID\":" + cboTax.SelectedValue + ",\"Description\":" + JsonConvert.ToString((desc.Substring(1))) + ",\"AmountDeb\":" + JsonConvert.ToString(txtTAX.Text) + ",\"AmountCre\":" + JsonConvert.ToString(0) + ",\"Ref\":" + JsonConvert.ToString(cbRef2.SelectedValue) + "}";
                        }
        ...
        private void chkWHT_CheckedChanged(object sender, EventArgs e)
        {
            ...
            //{
                ...
                pnlWHT.Visible = false;
                chkWHT.BackColor = Color.Honeydew;
        ...
        private void drawingDatePicker_Enter(object sender, EventArgs e)
        {
            string[] actDate = Fn.GetRecords("sp_ActiveDates");
            drawingDatePicker.MaxDate = Convert.ToDateTime(actDate[1]);
            drawingDatePicker.MinDate = Convert.ToDateTime(actDate[0]);
        }
        //bool whtp = true;
        ...
        private void chkTAX_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {chkTAX.Text = "Sales Tax";
                ...
                chkTAX.BackColor = Color.Honeydew;
                txtTotal.Text = txtAmount.Text;
                txtTAXp.Text = "0";
            }
        }
