...
namespace AccountSystem
{
    public partial class rptInwardGatePassRegis : Form
    {
        ...
        private static rptInwardGatePassRegis defaultInstance;
        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static rptInwardGatePassRegis Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new rptInwardGatePassRegis();
        ...
        {
            string gatekeeper;string vehicleNo;string BillNo;
            if (txtGateKeeper.Text=="" || txtGateKeeper.Text is null)
            {
                gatekeeper = "%";
            }
            else
            {
                gatekeeper = "%" + txtGateKeeper.Text + "%";
            }
            if (txtVehicleNo.Text == "" || txtVehicleNo.Text is null)
            {
                vehicleNo = "%";
            }
            else
            ...
            if (txtBillno.Text == "" || txtBillno.Text is null)
            {
                BillNo = "%";
            }
            else
            ...
            string[] SQL = { "sp_InwardRegister '" + datePicker1.Text + "','" + datePicker2.Text + "','"+ gatekeeper + "','"+ vehicleNo + "','"+ BillNo + "'", "SELECT CompanyID, Logo FROM TblCompany WHERE (CompanyID = 1)" };
