...
namespace AccountSystem
{
    public partial class frmBPV : Form
    {
        ...
        private static frmBPV defaultInstance;
        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static frmBPV Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new frmBPV();
        ...
        private void fillHeads_E()
        {
            ...
            //    Fn.fillCombo(cmboBank, "SELECT TblAccEntry.EntryAccID,TblAccEntry.EntryAccName+' '+right(REPLICATE('0',2) + cast(TblAccHead.AccHeadCode as varchar(2)),2)+ '-' + right(REPLICATE('0',3) + cast(TblAccSubHead.AccSubCode as varchar(3)),3)+ '-' + right(REPLICATE('0',4) + cast(TblAccEntry.EntryAccCode as varchar(4)),4) FROM TblAccHead INNER JOIN TblAccSubHead ON TblAccHead.AccHeadID = TblAccSubHead.AccHeadID INNER JOIN TblAccEntry ON TblAccSubHead.AccSubID = TblAccEntry.AccSubID WHERE (TblAccHead.CompanyID =" + MyModule.CompID + ") AND (TblAccEntry.AccType=3) AND (TblAccEntry.Stat = 1)");
            //}
            Fn.fillCombo(cboAcc, "SELECT TblAccEntry.EntryAccID,TblAccEntry.EntryAccName+' '+right(REPLICATE('0',2) + cast(TblAccHead.AccHeadCode as varchar(2)),2)+ '-' + right(REPLICATE('0',3) + cast(TblAccSubHead.AccSubCode as varchar(3)),3)+ '-' + right(REPLICATE('0',4) + cast(TblAccEntry.EntryAccCode as varchar(4)),4) FROM TblAccHead INNER JOIN TblAccSubHead ON TblAccHead.AccHeadID = TblAccSubHead.AccHeadID INNER JOIN TblAccEntry ON TblAccSubHead.AccSubID = TblAccEntry.AccSubID WHERE (TblAccHead.AccType <> N'Expenditure') AND (TblAccHead.CompanyID =" + MyModule.CompID + ") AND (TblAccEntry.Stat = 1) AND (TblAccEntry.AccType NOT IN (21, 22, 23))");
            cboAcc.Text = null; cbRef.Text = null;
        }
        private void frmBVoucher_Load(object sender, EventArgs e)
        {
            txtDr.Height = cboAcc.Height;
            txtNarr.Height = cboAcc.Height; Fn.fillCombo(cboWHTax, "SELECT  EntryAccID, EntryAccName FROM TblAccEntry WHERE (EntryAccName LIKE N'%w%tax%')");
            Fn.fillCombo(cboTax, "SELECT  EntryAccID, EntryAccName FROM TblAccEntry WHERE (EntryAccName LIKE N'%s%tax%')");
            Fn.fillCombo(cbLocation, "sp_OfficeLocations");
            cbLocation.SelectedValue = MyModule.BranchID;
            //Fn.GetNum(txtVoucherNo, "SELECT COUNT(*) AS V FROM TblVoucherHDR0 WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'BPV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");

            Fn.GetNum(txtVoucherNo, "SELECT COUNT(tbl.VoucherID) AS Voch FROM (SELECT VoucherID FROM TblVoucherDTL GROUP BY VoucherID) AS tbl INNER JOIN TblVoucherHDR ON tbl.VoucherID = TblVoucherHDR.VoucherID WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'BPV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");

            txtVoucherNo.Text = "BPV/" + txtVoucherNo.Text; fillHeads_R();
            Fn.fillCombo(cmboBank, "SELECT EntryAccID, EntryAccName FROM TblAccEntry WHERE (AccType = 3) AND (Stat = 1) order by EntryAccName");
        ...
        private void Amount()
        {
            ...
            {
                bdeb = 0;
                bcre = Convert.ToDouble(txtTotal.Text);
            ...
            {
                bdeb = Convert.ToDouble(txtTotal.Text);
        ...
        private Boolean GridChk()
        {
            ...
            try
            {
                ...
                {
                    Double a;
                    a = Convert.ToDouble(tblVoucherDTLDataGridView.Rows[i].Cells["Amt"].Value) / 2;
                    txtBankDesc.Text = tblVoucherDTLDataGridView.Rows[i].Cells["Description"].Value + ", " + txtBankDesc.Text;
                }
            }
            catch
            {
        ...
        private void tblVoucherDTLDataGridView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == 0)
                {
                    try
                    {
                        if (tblVoucherDTLDataGridView.Rows[e.RowIndex].Cells["Description"].Value == null && tblVoucherDTLDataGridView.Rows[e.RowIndex].Cells["AccountCode"].Value != null)
                        {
                            //if ((e.RowIndex+1) % 2 == 0)
                            //{
                            tblVoucherDTLDataGridView.Rows[e.RowIndex].Cells["Description"].Value = txtBankDesc.Text;
                            //}
                        }
                    }
                    catch
                ...
                try
                {
                    ...
                    {
                        sumcr = sumcr + Convert.ToDouble(tblVoucherDTLDataGridView.Rows[counter].Cells["Amt"].Value.Equals(System.DBNull.Value) ? 0 : tblVoucherDTLDataGridView.Rows[counter].Cells["Amt"].Value);
                        counter = counter + 1;
                    }
                }
                catch
                {
                    tblVoucherDTLDataGridView.Rows[counter].Cells["Amt"].Value = 0;
                }
        ...
        {
            Fn.GetNum(txtVoucherNo, "SELECT COUNT(*) AS V FROM TblVoucherHDR0 WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'BRV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (btnSave.Text == "&Add New")
            {
                ...
                clear();
                chkWHT.Checked = false; chkTAX.Checked = false;
                //Fn.GetNum(txtVoucherNo, "SELECT COUNT(*) AS V FROM TblVoucherHDR0 WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'BPV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");
                Fn.GetNum(txtVoucherNo, "SELECT COUNT(tbl.VoucherID) AS Voch FROM (SELECT VoucherID FROM TblVoucherDTL GROUP BY VoucherID) AS tbl INNER JOIN TblVoucherHDR ON tbl.VoucherID = TblVoucherHDR.VoucherID WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'BPV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");
                txtVoucherNo.Text = "BPV/" + txtVoucherNo.Text;
                bexist = false;
            ...
            else
            {
                if (ValidateControls())
                {
                    ...
                    {
                        MessageBox.Show("Please check Debit and Credit are not equal ", Application.CompanyName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        double DrAmt = 0; double CrAmt = 0;
                        string desc = "";
                        //double totAmtDr = 0; double totAmtCr = 0;
                        ...
                        items = items + "," + "{\"AccEntryID\":" + JsonConvert.ToString(cmboBank.SelectedValue) + ",\"Description\":" + JsonConvert.ToString((desc.Substring(1))) + ",\"AmountDeb\":" + JsonConvert.ToString(0) + ",\"AmountCre\":" + JsonConvert.ToString(txtAmount.Text) + ",\"Ref\":" + JsonConvert.ToString(0) + "}";
                        if (chkWHT.Checked)
                        {
                            items = items + "," + "{\"AccEntryID\":" + cboWHTax.SelectedValue + ",\"Description\":" + JsonConvert.ToString((desc.Substring(1))) + ",\"AmountDeb\":" + JsonConvert.ToString(0) + ",\"AmountCre\":" + JsonConvert.ToString(txtWHT.Text) + ",\"Ref\":" + JsonConvert.ToString("") + "}";
                        }
                        if (chkTAX.Checked)
                        {
                            items = items + "," + "{\"AccEntryID\":" + cboTax.SelectedValue + ",\"Description\":" + JsonConvert.ToString((desc.Substring(1))) + ",\"AmountDeb\":" + JsonConvert.ToString(0) + ",\"AmountCre\":" + JsonConvert.ToString(txtTAX.Text) + ",\"Ref\":" + JsonConvert.ToString("") + "}";
                        }
                        ...
                        string ID = Fn.GetRecords("sp_CreateBankVouchers0 '" + Data2Post.Replace("'", "''") + "'," + cbLocation.SelectedValue + ",'" + vCh + "','" + txtCheqNo.Text + "'")[0];
                        txtVid.Text = ID;

                        //////////////////////////////// to GL ///////////////////////////////////////////
                        Fn.Exec("UPDATE TblVoucherHDR0 SET VStatus = 2 WHERE (VoucherID = " + ID + ")");
                        ID = Fn.GetRecords("sp_CreateBankVouchers '" + Data2Post.Replace("'", "''") + "'," + cbLocation.SelectedValue + ",'" + vCh + "','" + txtCheqNo.Text + "'")[0];
                        txtVid.Text = ID;
                        ...
                        //        cmd.Parameters.AddWithValue("@AccEntryID", tblVoucherDTLDataGridView.Rows[i].Cells["AccountCode"].Tag);
                        //        cmd.Parameters.AddWithValue("@Description", tblVoucherDTLDataGridView.Rows[i].Cells["Description"].Value.Equals(null) ? "-" : tblVoucherDTLDataGridView.Rows[i].Cells["Description"].Value);
                        //        cmd.Parameters.AddWithValue("@AmountDeb", 0);
                        //        cmd.Parameters.AddWithValue("@AmountCre", tblVoucherDTLDataGridView.Rows[i].Cells["Amt"].Value);
                        ...
                        //        cmd.Parameters.AddWithValue("@AccEntryID", tblVoucherDTLDataGridView.Rows[i].Cells["AccountCode"].Tag);
                        //        cmd.Parameters.AddWithValue("@Description", tblVoucherDTLDataGridView.Rows[i].Cells["Description"].Value.Equals(null) ? "-" : tblVoucherDTLDataGridView.Rows[i].Cells["Description"].Value);
                        //        cmd.Parameters.AddWithValue("@AmountDeb", tblVoucherDTLDataGridView.Rows[i].Cells["Amt"].Value);
        ...
        private void btnAddRow_Click(object sender, EventArgs e)
        {
            ...
            tblVoucherDTLDataGridView.Rows[tblVoucherDTLDataGridView.RowCount - 1].Cells["AccountCode"].Value = cboAcc.Text;
            tblVoucherDTLDataGridView.Rows[tblVoucherDTLDataGridView.RowCount - 1].Cells["AccountCode"].Tag = cboAcc.SelectedValue;
            ...
            tblVoucherDTLDataGridView.Rows[tblVoucherDTLDataGridView.RowCount - 1].Cells["Amt"].Value = txtDr.Value;
            tblVoucherDTLDataGridView.Height = tblVoucherDTLDataGridView.Height + 22;
            cboAcc.Text = null; cboAcc.SelectedIndex = -1; cbRef.Text = null; txtNarr.Text = ""; txtDr.Value = 0; cbRef.Text = ""; txtExpDT.Checked = false;
            cboAcc.Focus();
            int counter = 0;
            Double sumcr = 0;
            try
        ...
        private void chkWHT_CheckedChanged_1(object sender, EventArgs e)
        {
            if (chkWHT.Checked)
            {
                //chkWHT.Tag = 0;
                //txtTotal.Text = "0";
                //txtWHT.Text = "0";
                chkWHT.BackColor = Color.LightPink;
                chkWHT.Text = "";
                pnlWHT.Visible = true;
                //object[] WHT;
                //WHT = Fn.GetRecords("SELECT WHTax FROM tblMonthCloseing where CompID=" + MyModule.CompID + "");
                ...
                if (!string.IsNullOrEmpty(txtAmount.Text))
                {
                    txtWHT.Text = (Convert.ToDouble(txtWHTp.Text) * Convert.ToDouble(txtAmount.Text)).ToString();
                    txtTotal.Text = (Convert.ToDouble(txtWHT.Text) + Convert.ToDouble(txtTAX.Text) + Convert.ToDouble(txtAmount.Text)).ToString();
                }
            }
            else
            {
                chkWHT.Text = "WithHolding Tax";
                pnlWHT.Visible = false;
                chkWHT.BackColor = Color.FloralWhite;
        ...
        private void chkTAX_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                pnlTAX.Visible = true;
                chkTAX.BackColor = Color.Pink;
                chkTAX.Text = "";
                if (!string.IsNullOrEmpty(txtAmount.Text))
                {
                    txtTAX.Text = (Convert.ToDouble(txtTAXp.Text) * Convert.ToDouble(txtAmount.Text)).ToString();
                    txtTotal.Text = (Convert.ToDouble(txtTAX.Text) + Convert.ToDouble(txtWHT.Text) + Convert.ToDouble(txtAmount.Text)).ToString();
                }
            }
            else
            {
                chkTAX.Text = "Sales Tax";
                pnlTAX.Visible = false;
                chkTAX.BackColor = Color.FloralWhite;
                txtTotal.Text = txtAmount.Text;
                txtTAXp.Text = "0";
            }
        }
        private void txtWHTp_ValueChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtAmount.Text))
            {
                //if (!whtp)
                //{
                txtWHT.Text = (Convert.ToDouble(txtAmount.Text) * Convert.ToDouble(txtWHTp.Text) / 100).ToString();
                txtTotal.Text = (Convert.ToDouble(txtWHT.Text) + Convert.ToDouble(txtTAX.Text) + Convert.ToDouble(txtAmount.Text)).ToString();
                //}whtp = false;
            }
        }
        private void txtTAXp_ValueChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtAmount.Text))
            {
                //if (!whtp)
                //{
                txtTAX.Text = (Convert.ToDouble(txtAmount.Text) * Convert.ToDouble(txtTAXp.Text) / 100).ToString();
                txtTotal.Text = (Convert.ToDouble(txtWHT.Text) + Convert.ToDouble(txtTAX.Text) + Convert.ToDouble(txtAmount.Text)).ToString();
                //}whtp = false;
            }
        }
        private void txtWHT_ValueChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtAmount.Text))
            {
                //if (whtp)
                //{
                txtTotal.Text = (Convert.ToDouble(txtWHT.Text) + Convert.ToDouble(txtTAX.Text) + Convert.ToDouble(txtAmount.Text)).ToString();
                txtWHTp.Text = (Convert.ToDouble(txtWHT.Text) / Convert.ToDouble(txtAmount.Text) * 100).ToString();
                //}
                //whtp = true;
            }
        }
        private void txtTAX_ValueChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtAmount.Text))
            {
                ...
                //{
                txtTotal.Text = (Convert.ToDouble(txtWHT.Text) + Convert.ToDouble(txtTAX.Text) + Convert.ToDouble(txtAmount.Text)).ToString();
                txtTAXp.Text = (Convert.ToDouble(txtTAX.Text) / Convert.ToDouble(txtAmount.Text) * 100).ToString();
                //}
                //whtp = true;
            }
        }
