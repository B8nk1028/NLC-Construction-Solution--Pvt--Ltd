...
{
    public partial class frmMakeSaleInvoice : Form
    {
        private MyModule Fn = new MyModule();
        string[] cData;
        public frmMakeSaleInvoice()
        {
            InitializeComponent();
        ...
        private static frmMakeSaleInvoice defaultInstance;
        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static frmMakeSaleInvoice Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new frmMakeSaleInvoice();
        ...
        private void frmPurchaseOrder_Load(object sender, EventArgs e)
        {
            ...
            this.sODetailsTableAdapter.Fill(this.dataSet1.SODetails,0);
            ...
            txtInvoicNo.Text = Fn.GetRecords("SELECT isnull(MAX(convert(int,Sale_bill_no)),0) +1 FROM tbl_Sale WHERE (BranchID = " + cbLocation.SelectedValue + ")")[0];
            ...
            this.sOHeadTableAdapter.Fill(this.dataSet1.SOHead,Convert.ToInt32(cbLocation.SelectedValue),0);
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (grdBill.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < grdBill.Rows.Count; i++)
                {
                    if (Int32.Parse(grdBill.Rows[i].Cells["gtxtremaining"].Value.ToString())< (Int32.Parse(grdBill.Rows[i].Cells["txtInvoiced"].Value==null?"0": grdBill.Rows[i].Cells["txtInvoiced"].Value.ToString())))
                    {
                        MessageBox.Show("Invoiced QTY is more than dispatched QTY.", "NLC CS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        btnGen.Enabled = true;
                        return;
                    }
                    //grdBill.Rows[i].Cells["Remaings"].Value = Int32.Parse(grdBill.Rows[i].Cells["Remaings"].Value.ToString()) - Int32.Parse(grdBill.Rows[i].Cells["txtInvoiced"].Value.ToString());
                    ...
                    grdBill.Rows[i].Cells["colInvoiced"].Value = Int32.Parse(grdBill.Rows[i].Cells["colInvoiced"].Value.ToString()) + (Int32.Parse(grdBill.Rows[i].Cells["txtInvoiced"].Value == null ? "0" : grdBill.Rows[i].Cells["txtInvoiced"].Value.ToString()));
