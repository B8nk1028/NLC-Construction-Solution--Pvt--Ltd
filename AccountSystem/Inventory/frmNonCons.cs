...
{
    public partial class frmNonCons : Form
    {
        ...
        private static frmNonCons defaultInstance;
        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static frmNonCons Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new frmNonCons();
        ...
        private void frmPurchaseOrder_Load(object sender, EventArgs e)
        {
            ...
            Fn.fillCombo(cboProducts, "sp_ProductListNonConsumable '" + cboStores.SelectedValue + "'");
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            ...
            if (grdBill.Rows.Count > 0)
            {
                    ...
                    for (int i = 0; i < grdBill.Rows.Count; i++)
                {
                    ...
                    items = items + "," + "{\"prod_id\":\"" + grdBill.Rows[i].Cells["gtxtItem"].Tag.ToString() + "\",\"prod_qty\":\"" + grdBill.Rows[i].Cells["gtxtQty"].Value.ToString() + "\",\"P_Remarks\":\"" + grdBill.Rows[i].Cells["gtxtUsedfor"].Value.ToString() + "\",\"PackingDetail\":\"" + grdBill.Rows[i].Cells["gtxtRemarks"].Value.ToString() + "\"}";
                }
                string Data2Post = "{\"DeptID\":\"" + cboVendor.SelectedValue + "\",\"DemandNo\":\"" + txtDemandNo.Text + "\",\"StoreID\":\"" + cboStores.SelectedValue + "\",\"IssueDT\":" + JsonConvert.ToString(dtIssuDate.Text) + ",\"Created\":\"" + MyModule.UserName + "|" + DateTime.Now + "|" + MyModule.PCInfo + "\",\"CompID\":" + MyModule.CompID + ",\"items\":[" + items.Substring(1) + "]}";
                string ID = Fn.GetRecords("sp_NonConsumableItems '" + Data2Post + "'," + cbLocation.SelectedValue)[0];
                btnPrint.Tag = ID;
                if (ID == "a")
                {
