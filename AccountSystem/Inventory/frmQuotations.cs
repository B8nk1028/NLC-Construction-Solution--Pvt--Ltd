...
namespace AccountSystem
{
    public partial class frmQuotations : Form
    {
        ...
        private static frmQuotations defaultInstance;
        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static frmQuotations Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new frmQuotations();
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter sale contract number", "GM Soft", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (grdBill.Rows.Count > 0 && cboVendor.SelectedValue != null)
            {
                ...
                for (int i = 0; i < grdBill.Rows.Count; i++)
                {
                    items = items + "," + "{\"productid\":\"" + grdBill.Rows[i].Cells["gdv_prod_name"].Value + "\",\"rate\":\"" + grdBill.Rows[i].Cells["prchse_prod_rate"].Value.ToString() + "\",\"qty\":\"" + grdBill.Rows[i].Cells["Prchse_prod_qty"].Value.ToString() + "\",\"disc\":\"" + grdBill.Rows[i].Cells["Discount"].Value.ToString() + "\"}";
                }
                ...
                string ID = Fn.GetRecords("sp_Quotations '" + Data2Post + "'," + cbLocation.SelectedValue)[0];
                btnPrint.Tag = ID;
                if (ID == "a")
                {
        ...
        private void Calc()
        {
            ...
            decimal Discount = 0;
            double total = 0; double ntotal = 0; double tax = 0;
            ...
            for (int i = 0; i < grdBill.Rows.Count; i++)
            {
                ...
                {
                    total = total + Convert.ToDouble(grdBill.Rows[i].Cells["Prchse_Prod_Amount"].Value);
                    ntotal = ntotal + Convert.ToDouble(grdBill.Rows[i].Cells["GAmount"].Value);
                    tax = tax + Convert.ToDouble(grdBill.Rows[i].Cells["Discount"].Value);
                    txttAmount.Text = Convert.ToDouble(ntotal).ToString("#,#.00");
                    txtTax.Text = Convert.ToDouble(tax).ToString("#,#.00");
                    txtTotal.Text = Convert.ToDouble(total).ToString("#,#.00");
                }
            }
        ...
        {
            if (cboProducts.Text == "" || (txtQty.Value + txtRate.Text) == "00")
            {
            ...
            grdBill.Rows[grdBill.RowCount - 1].Cells[1].Value = txtRate.Text;
            grdBill.Rows[grdBill.RowCount - 1].Cells[2].Value = txtQty.Text;
            grdBill.Rows[grdBill.RowCount - 1].Cells[3].Value = txtGAmount.Text;
            grdBill.Rows[grdBill.RowCount - 1].Cells[4].Value = txtDAmt.Value;
        ...
        private void txtDAmt_ValueChanged_1(object sender, EventArgs e)
        {
            Calc();
        }
    }
