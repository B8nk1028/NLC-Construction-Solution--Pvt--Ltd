...
{
    public partial class frmCVoucher : Form
    {
        ...
        private static frmCVoucher defaultInstance;
        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static frmCVoucher Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new frmCVoucher();
        ...
        {
            Fn.GetNum(txtVoucherNo, "SELECT COUNT(*) AS V FROM TblVoucherHDR WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'CPV/%') AND (CompanyID =" + MyModule.CompID + ")AND (BranchID =" + cbLocation.SelectedValue + ")");
            txtVoucherNo.Text = "CPV/" + txtVoucherNo.Text;
            fillHeads_R();
        }
        private void RadioBankDeposit_CheckedChanged(object sender, EventArgs e)
        {
            Fn.GetNum(txtVoucherNo, "SELECT COUNT(*) AS V FROM TblVoucherHDR WHERE (convert(date,EntryDate) between '" + MyModule.ActiveYear + "' and (DATEADD(MONTH,12, convert(datetime,'" + MyModule.ActiveYear + "'))-1)) AND (VoucherNo LIKE N'CRV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");
            ...
            else
            {
                if (ValidateControls())
                {
                    ...
                    else
                    {
                        ...
                        string ID = Fn.GetRecords("sp_CreateCashVouchers '" + Data2Post + "'," + cbLocation.SelectedValue + ",'" + vCh + "'")[0];
                        txtVid.Text = ID;
                        if (ID == "a")
                        {
                        ...
                        //{
                        //    txtVoucherNo.Clear();
                        //    Fn.GetNum(txtVoucherNo, "SELECT COUNT(*) AS V FROM TblVoucherHDR WHERE (CONVERT(varchar, MONTH(EntryDate)) + CONVERT(varchar, YEAR(EntryDate)) = '" + MyModule.AMonth + "') AND (VoucherNo LIKE N'CPV/%') AND (CompanyID =" + MyModule.CompID + ") AND (BranchID =" + cbLocation.SelectedValue + ")");
                        //    txtVoucherNo.Text = "CPV/" + txtVoucherNo.Text;
                        //    txtVid.Text = Fn.ExenID("insert into TblVoucherHDR (VoucherNo,EntryDate,Created,Modified,Repli,CompanyID,BranchID) values('" + txtVoucherNo.Text + "','" + entryDatePicker.Text + "','" + MyModule.UserName + "|" + DateTime.Now + "|" + MyModule.PCInfo + "','" + MyModule.UserName + "|" + DateTime.Now + "|" + MyModule.PCInfo + "',0," + MyModule.CompID + "," + cbLocation.SelectedValue + ") SELECT @@IDENTITY").ToString();
                        //    //Fn.Exec("insert into TblVoucherDTL (VoucherID,AccEntryID,Description,AmountDeb,AmountCre,Repli) values('" + txtVid.Text + "','" + cmboEntry.SelectedValue + "','" + txtEntryDesc.Text + "'," + entDr + "," + entCr + ",0)");
                        //    //------------------------------
        ...
        private void fillHeads_R()
        {
            ...
            //}
            Fn.fillCombo(cboAcc, "SELECT TblAccEntry.EntryAccID,TblAccEntry.EntryAccName+' '+right(REPLICATE('0',2) + cast(TblAccHead.AccHeadCode as varchar(2)),2)+ '-' + right(REPLICATE('0',3) + cast(TblAccSubHead.AccSubCode as varchar(3)),3)+ '-' + right(REPLICATE('0',4) + cast(TblAccEntry.EntryAccCode as varchar(4)),4) FROM TblAccHead INNER JOIN TblAccSubHead ON TblAccHead.AccHeadID = TblAccSubHead.AccHeadID INNER JOIN TblAccEntry ON TblAccSubHead.AccSubID = TblAccEntry.AccSubID WHERE (TblAccHead.AccType <> N'Revenue') AND (TblAccHead.CompanyID =" + MyModule.CompID + ") AND (TblAccEntry.Stat = 1)");
        ...
        private void cmboBank_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txtBOpening.Text = String.Format("{0:#,#}", Convert.ToInt32(txtBOpening.Text));
                //------------------------
            }
            catch
            {
